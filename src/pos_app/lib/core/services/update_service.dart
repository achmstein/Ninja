...
/// Keeps the till on the newest build from the platform's download page.
///
/// On start and every few hours it asks the café's stack where the page is
/// (`/api/tenant` → `appsUrl`), reads `ninja-pos.json` there and, when the
...
/// Any other time the settings screen offers the button. Sales waiting to
/// replay hold an install back.
class UpdateNotifier extends Notifier<UpdateState> {
  static const _channel = MethodChannel('com.ninja.pos/update');
  static const _file = 'ninja-pos';
  static const _interval = Duration(hours: 6);

  /// Something is in flight that a restart would lose (the till's offline sales); set by the app
