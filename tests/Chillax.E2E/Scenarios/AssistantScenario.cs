...
/// on a bilingual field fills in the other language (both directions, with
/// a category suggestion on a new menu item), and "Scan receipt" turns a
/// photo into proposed lines that the review sheet then turns into a real
/// delivery — a new stock item created, the purchase received, Finance
/// invoiced. The AppHost runs the services with the scripted fake under
/// test, so the answers are deterministic and no key or network is needed.
/// </summary>
public sealed class AssistantScenario(ChillaxApp app, DaySetup day) : ScenarioBase(app, day)
{
    ...
    [Fact]
    public async Task Fills_in_the_other_language_and_reads_a_receipt_into_a_delivery()
    {
        // 1. English in, Arabic out — and a category for a new menu item.
        Step("Ask the assistant for the Arabic of a new menu item");
        var localized = await Owner.LocalizeAsync(MenuItem, new LocalizedText("E2E Mango Juice"), Ct,
            description: new LocalizedText("Fresh mango, blended to order"), suggestCategory: true);
        Assert.Equal("E2E Mango Juice", localized.Name.En);
        Assert.Equal("E2E Mango Juice (تجريبي)", localized.Name.Ar);
        Assert.Equal("Fresh mango, blended to order (تجريبي)", localized.Description!.Ar);
        Assert.NotNull(localized.SuggestedCatalogTypeId);
        Assert.Equal(["name.ar", "description.ar", "catalogTypeId"], localized.Filled);
        ...
        // 3. Both sides already filled: the endpoint says so instead of guessing.
