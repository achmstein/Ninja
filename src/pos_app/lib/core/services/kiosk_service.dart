...
const _enabledKey = 'pos.kiosk.enabled';

/// What the tablet can do and is doing about being a till and nothing else
class KioskStatus {
  ...
  /// The cashier asked for kiosk mode; re-applied on every resume
  final bool enabled;
...
/// Enabling kiosk mode is a per-till choice kept in preferences; without
/// device-owner rights Android only offers its screen-pinning prompt, so the
/// settings screen says which of the two it is.
class KioskService {
  static const _channel = MethodChannel('com.chillax.pos/kiosk');
  ...
  /// On start and on every resume: a till that asked to be pinned stays
