...
/// queued published only after the commit. Both ways into Payroll go through
/// it: MediatR commands via <see cref="TransactionBehavior{TRequest, TResponse}"/>,
/// and (from Phase 2) the bus handler that records till pay-outs.
/// </summary>
public class PayrollTransaction(
    PayrollContext dbContext,
    IPayrollIntegrationEventService integrationEvents,
    ILogger<PayrollTransaction> logger)
{
        ...
        await strategy.ExecuteAsync(async () =>
        {
            ...
            using (logger.BeginScope(new List<KeyValuePair<string, object>> { new("TransactionContext", transaction.TransactionId) }))
            {
                ...
                catch (DbUpdateConcurrencyException ex)
                {
                    ...
                    throw new PayrollDomainException("Someone else changed this a moment ago. Reload and try again.", ex);
                }
