 ...
 * only ever see a splash; a card with a retry button appears only if the
 * sign-in actually fails. Same flow as the other web apps.
 */
function SignIn() {
  const t = useT()
  const auth = useAuth()
  ...
  const beginSignIn = () => {
    ...
    auth.signinRedirect(loginPageParams(resolvedTheme, 'en'))
  }
