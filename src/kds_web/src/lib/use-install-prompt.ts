...
/**
 * Installability of the kitchen display as a PWA (same hook as client_web).
 *
 * Chromium browsers (Android Chrome, desktop Chrome/Edge) announce it with
 * `beforeinstallprompt` — once, and early, often before React has mounted —
 * so the listener lives at module scope and the event is parked here until
 * a component asks for it. iOS/iPadOS has no prompt at all: there the app
 * can only explain Safari's "Share → Add to Home Screen".
 */
 ...
 * whatever the user picks — Chrome fires a fresh one when it is willing to
 * ask again.
 */
