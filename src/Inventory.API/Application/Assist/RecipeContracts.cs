...
/// <param name="OptionIds">Empty for the base recipe; else every option the line needs chosen.</param>
public sealed record ProposedRecipeLine(int? StockItemId, string? NewItemKey, decimal Quantity, IReadOnlyList<int> OptionIds);
...
public sealed record ExtractedRecipeLine(int StockItemId, string NewItemKey, decimal Quantity, IReadOnlyList<int> OptionIds);
