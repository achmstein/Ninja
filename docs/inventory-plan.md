...
- A **slot** has a **default** (stock item + quantity for a sale where the customer changes nothing, or *none*: the slot only appears for some choice) and **overrides** keyed on one option or a combination (replace the item and/or the quantity, or set *none*).
- A **size** is an override like any other: دبل is the coffee slot's line for the double option with 14 g instead of 7. *(Until later on 2026-09-15 the recipe also had scale factors on options — دبل ×2 — multiplying the slots flagged scalable; one idea in two shapes, and the seam between them is where a bug lived, so the factor went: migration `DropRecipeScales` writes each factor into per-size lines and drops `RecipeScale` and `Scalable`. The builder, the advanced editor, the API and the assistant all speak amounts per size now.)*
- **Resolution** for a sale with chosen options C: *for each slot, take the most specific override whose options are all in C (most options named; else the default).* An override replaces within its slot; nothing is additive across a slot, so an uncovered combination falls back to the default instead of to nothing. The only way to deduct nothing is to say *none*.
- **Storage**: `RecipeLine` gains `Slot` and `IsNone`. Migration `RecipeSlotsAndScales` is additive and **behaviour-preserving**: every existing line becomes its own slot (a base line the slot's default, an option line an override with no default), so the old additive semantics hold until a recipe is restructured. The old constructor and `RecipeLineInput` without a slot keep working the same way.
...
The builder asks three questions per ingredient: which item (fixed, or a table by the groups that decide it), how much (fixed, or per choice of one group), when (always, or only with some choices). They compile into a slot: one override per full combination of the deciding groups plus, as the default, the combination the till sends when nothing is touched — each group's default option, the first option of a required group without one, and no option at all for an add-on group or an optional group without a default, so such an ingredient has no default line and is deducted only when its option is chosen. An add-on group gets overrides only for the choices the ingredient is deducted for: a sale may carry several of its options, and a *none* override for a sibling would tie with the add-on's own and win by order (`recipe-cards.ts`, under vitest). The advanced editor shows a recipe as rows, one per slot — *7 g of بن وسط سادة · scales with size · depends on التحميص × التحويجة* — and, under a row that depends on groups, a grid (one group: a list; two: rows × columns) with the item and quantity per cell, an empty cell meaning "same as the default", a cell set to *none* meaning nothing. The "try it" preview picks options like the cashier and shows the resolved deduction. The bulk review sheet uses the same editor per item, and the assistant proposes in this shape.
...
| `Recipe` PK CatalogItemId | Lines of (`Slot`, StockItemId, Quantity per unit sold, `OptionIds` integer[], `IsNone`). Lines with the same `Slot` are one slot: the line with no options is its default, the others its overrides, keyed on one option or a combination (a bag that exists for medium roast + spiced is one override tied to both; a double is the coffee line again with 14 g). `Explode(units, chosenOptionIds)` resolves each slot to its most specific applicable override (else the default) and skips *none* (D6, migrations `RecipeSlotsAndScales` and `DropRecipeScales`, 2026-09-15). Only single-option overrides drive per-option sold-out. |
...
**Also 2026-09-15 — slots (D6):** recipes are edited as slots with defaults and overrides; every existing recipe migrated line-for-line into its own slots and deducts as before.
...
**Fixed later the same day — the builder's compile:** it took every group's first option as the standard choice, so an add-on (whipped cream) was deducted for a sale that chose nothing from its group and an optional group without a default deducted its first option when the cashier left it alone; it wrote a *none* override for every sibling add-on, which tied with the chosen add-on's own override when two were picked and won by order, so the second was never deducted; and a recipe whose size factors could not become amounts per size (the amount already hung on another group) lost the factors silently on save. The cards' compile and read-back now live in `admin_web/src/features/menu/recipe-cards.ts`, pure and under vitest (`npm test` in `admin_web`, the SPA's first test suite): the standard choice of a group is its default, its first option only when the group is required (what the customer app picks), and nothing for an add-on group or an optional one without a default; an add-on group gets overrides only for the choices the ingredient is deducted for; a row the cards cannot size stays a custom rule and keeps its factor. A recipe saved in the old shape reads back as the card that wrote it and is repaired on the next save.

**Later still (2026-09-15) — one shape for a size:** the scale factor went (see D6): `RecipeScale` and `RecipeLine.Scalable` are gone from the domain, the API, the read models and the assistant's contract, the proposer is told a size is a line in the ingredient's slot, and migration `DropRecipeScales` writes each factor into per-size lines before dropping the table and the column. The SPA lost the size-factor row, the "grows with size" tick and the read-back conversion; the cards' "amount depends on the size" is the only way to say a size.
