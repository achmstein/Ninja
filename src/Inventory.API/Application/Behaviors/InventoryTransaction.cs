...
/// queued published only after the commit. Both ways into Inventory go through
/// it: MediatR commands via <see cref="TransactionBehavior{TRequest, TResponse}"/>,
/// and the bus handler that deducts stock for confirmed orders.
/// </summary>
public class InventoryTransaction(
    InventoryContext dbContext,
    IInventoryIntegrationEventService integrationEvents,
    ILogger<InventoryTransaction> logger)
{
        ...
        await strategy.ExecuteAsync(async () =>
        {
            ...
            using (logger.BeginScope(new List<KeyValuePair<string, object>> { new("TransactionContext", transaction.TransactionId) }))
            {
                ...
                catch (DbUpdateConcurrencyException ex)
                {
                    ...
                    // one stock item at once. The loser is told and
                    ...
                    throw new InventoryDomainException("Someone else changed this a moment ago. Reload and try again.", ex);
                }
