...
import 'package:kds_app/core/config/tenant_connection.dart';
...
void main() {
  group('the hosts an address means', () {
    ...
    test("the cafÃ©'s own host first, then its api. host", () {
  ...
  group('probing', () {
    final tenant = {
      'name': {'en': 'Lucaffe', 'ar': 'Ù„ÙˆÙƒØ§ÙÙŠÙ‡'},
      'auth': {'authority': 'https://auth.ninja.app/realms/lucaffe'},
    };

    test('keeps the first host that is a cafÃ©, with its realm', () async {
    ...
    test('says why when no host answers, is not a cafÃ©, or is paused', () async {
      expect(
