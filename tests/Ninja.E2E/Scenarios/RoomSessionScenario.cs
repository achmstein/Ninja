...
/// </summary>
public sealed class RoomSessionScenario(NinjaApp app, DaySetup day) : ScenarioBase(app, day)
{
    private Task<PlaceView> RoomAsync(string nameEn) => Cashier.PlaceAsync(nameEn, Ct);

    /// <summary>One line of a completed stay's Costs, by rate option.</summary>
    private static (decimal Hours, decimal Cost) CostOf(RecordedEvent completed, string optionCode)
    {
        var line = completed.Array("Costs").FirstOrDefault(c => c.GetProperty("OptionCode").GetString() == optionCode);
        return line.ValueKind == System.Text.Json.JsonValueKind.Undefined
            ? (0m, 0m)
            : (line.GetProperty("Hours").GetDecimal(), line.GetProperty("Cost").GetDecimal());
    }
    ...
    public async Task Walk_in_member_order_end_settle_and_the_cancel_paths()
    {
        ...
        Assert.Contains(await Cashier.OpenStaysAsync(Ct), s => s.Id == sessionId);
        ...
        var session = (await Cashier.StayAsync(sessionId, Ct))!;
        ...
        Assert.DoesNotContain(await Cashier.OpenStaysAsync(Ct), s => s.Id == sessionId);
        ...
        await Cashier.EndStayAsync(emptySession, Ct);
        ...
        await Cashier.CancelStayAsync(lateSession, Ct);
    ...
    public async Task A_quarter_hour_session_bills_room_time()
    {
        ...
        await Cashier.ChangeStayOptionAsync(sessionId, Codes.RateOption.Multi, Ct);

        var end = Step("End after ~8 minutes of Single: one quarter hour billed, the Multi seconds not");
        await Cashier.EndStayAsync(sessionId, Ct);
        var completed = await ExpectEventAsync(end, "SessionCompleted", e => e.Int("ReservationId") == sessionId);
        var single = CostOf(completed, Codes.RateOption.Single);
        var multi = CostOf(completed, Codes.RateOption.Multi);
        Assert.Equal(0.25m, single.Hours);
        Assert.Equal(0m, multi.Hours);
        Assert.Equal(Money.Round(0.25m * room1.Rate(Codes.RateOption.Single)), single.Cost);
        Assert.Equal(0m, multi.Cost);
        Assert.Equal(single.Cost, completed.Dec("TotalCost"));
