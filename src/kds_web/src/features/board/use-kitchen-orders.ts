 ...
 * X-Branch-Id header every request carries. SignalR is the primary update
 * path (see use-kitchen-notifications); the poll is only a fallback, and a
 * short one, because a card that is not there yet is a customer waiting.
 */
export function useKitchenOrders() {
  const query = useQuery({
    ...getKitchenOrdersOptions({ query: { 'api-version': API_VERSION } }),
