 ...
 * table). A payment's `amount` is its share of the bill; its fee rides
 * on top and never reduce what is left. Only a Paid one counts: a
...
export function onlineSummary(
  ...
  payments: readonly Pick<OnlinePaymentView, 'amount' | 'status'>[] | null | undefined
): OnlineSummary {
  const list = payments ?? []
