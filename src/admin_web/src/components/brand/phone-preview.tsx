...
import { logoFor, wordmarkFor, type Scheme } from '@/lib/brand-slots'
...
export type PreviewDraft = BrandThemeInput & {
  ...
  /** The saved brand, for its images; the draft has no images of its own */
  brand: Brand
  currency: string
}
 ...
 * `.dark`, whatever the admin is set to.
 */
export function PhonePreview({
  ...
  const wordmark = wordmarkFor(draft.brand, language, scheme)
  const logo = logoFor(draft.brand, scheme)
  const items = [
    ...
    <PhoneFrame scheme={scheme} className={className}>
      ...
      >
        <div className='flex items-center gap-2 border-b px-4 py-3'>
          ...
          <Badge variant='secondary' className='ms-auto'>{t('initialBranches')}</Badge>
        </div>
        <div className='flex-1 space-y-3 overflow-hidden p-4'>
          ...
          {items.map((item) => (
            <Card key={item.name} className='flex-row items-center gap-3 p-3'>
              ...
              <Button type='button' size='sm'>{t('previewAdd')}</Button>
            </Card>
          ))}
        </div>
