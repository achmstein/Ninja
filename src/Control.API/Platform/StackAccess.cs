...
/// <summary>The images a brand is made of, named as the stack names them: six drawings and the cover photo.</summary>
public static class BrandImageSlots
{
    public static readonly string[] All = ["logo", "logo-dark", "wordmark-en", "wordmark-en-dark", "wordmark-ar", "wordmark-ar-dark", "cover"];
    ...
    public async Task<HttpResponseMessage> SendAsync(Tenant tenant, HttpMethod method, string pathAndQuery, HttpContent? content, StackAuth auth, CancellationToken ct, int? branchId = null)
    {
        ...
        if (path == "/api/tenant")
        {
            ...
            if (method == HttpMethod.Put && content is not null)
            {
                ...
                foreach (var key in new[] { "name", "primaryColor", "customerUrl", "features", "theme", "locale", "businessType", "guestOrdersAnywhere" })
                {
